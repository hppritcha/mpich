...
!
subroutine MPI_Type_contiguous_x_f08(count, oldtype, newtype, ierror)
    use :: mpi_f08, only : MPI_Datatype
    use, intrinsic :: iso_c_binding, only : c_int
    use :: mpi_c_interface, only : c_Datatype
    use :: mpi_c_interface, only : MPIR_Type_contiguous_x_c
    ...
    if (c_int == kind(0)) then
        ierror_c = MPIR_Type_contiguous_x_c(count, oldtype%MPI_VAL, newtype%MPI_VAL)
    else
        count_c = count
        oldtype_c = oldtype%MPI_VAL
        ierror_c = MPIR_Type_contiguous_x_c(count_c, oldtype_c, newtype_c)
